...
(parenscript:defpsmacro dom-select (id name selected lst)
...
(defun add-google-maps-markers ()
  (ps:with (map markers infowindows) 
    (ps:defun init-map ()
      (setf map (google.maps.Map
		 (google.maps.get-element-by-id "map")
                 (google.maps.map-options :zoom 8
                                          :center (google.maps.lat-lng :lat -34.397 :lng 150.644))))
      
      ;; Add the first marker
      (setf markers (list
		     (google.maps.Marker
		      (google.maps.marker-options :position (google.maps.lat-lng :lat -34.397 :lng 150.644)
                                                                         :map map))))

      ;; Add the second marker
      (push (google.maps.Marker (google.maps.marker-options :position (google.maps.lat-lng :lat -34.5 :lng 150.8)
                                                           :map map))
            markers)

      ;; Add the third marker
      (push (google.maps.Marker (google.maps.marker-options :position (google.maps.lat-lng :lat -34.3 :lng 150.5)
                                                           :map map))
            markers)

      ;; Add info windows for each marker
      (setf infowindows (list
			 (google.maps.InfoWindow
			  (google.maps.info-window-options :content (format nil "<p>Marker 1 Location: ~a</p>" (ps:lat-lng-get-position (first markers)))))
                              (google.maps.InfoWindow (google.maps.info-window-options :content (format nil "<p>Marker 2 Location: ~a</p>" (ps:lat-lng-get-position (second markers)))))
                              (google.maps.InfoWindow (google.maps.info-window-options :content (format nil "<p>Marker 3 Location: ~a</p>" (ps:lat-lng-get-position (third markers)))))))

      ;; Add click event listeners for each marker
      (dotimes (i (length markers))
        (let ((marker (nth i markers))
              (infowindow (nth i infowindows)))
          (google.maps.event.add-listener marker "click" (lambda () (google.maps.infowindow-open infowindow map))))))))
